  ...
  try {
    ...
    if (messageType === 'text') {
      ...
      if (userState && userState.isAskingQuestion) {
        ...
        responseMessages.push(await sendMessage(req.sessionId, await sendQuestionToAPI(req.sessionId, messageText, userLanguage)));
      ...
      if (messageContent.list_reply?.id.startsWith('lang_')) {
        ...
        responseMessages = await selectService(req.sessionId, selectedLanguageCode);
      } else {
        ...
        switch (serviceId) {
          ...
          case 'ask_yes':
            ...
            responseMessages.push(await sendMessage(req.sessionId, await textToTextTranslationNMT('Please type your question', userLanguage)));
            break;
          case 'main_menu':
          ...
          case 'trust_officials':
            ...
            responseMessages = await handleButtonReply(req.sessionId, serviceId, messageContent, userLanguage);
            break;
          default:
...
async function selectService(sessionId, languageCode) {
  ...
  const [headerMessage, bodyMessage, footerMessage, btnMessage, radioMessage] = await Promise.all([
    headerPromise,
    bodyPromise,
    footerPromise,
    btnPromise,
...
//   console.log(aboutTemple);
  ...
  return [
    ...
    await sendRadio(sessionId, aboutTemple.buttons), ...(await goBackToMainMenu(sessionId, userLanguage))
  ];
}
...
async function goBackToMainMenu(sessionId, userLanguage) {
  ...
  return [await sendMessage(sessionId, mainMenuMessage), await sendRadio(sessionId, buttons)];
}
